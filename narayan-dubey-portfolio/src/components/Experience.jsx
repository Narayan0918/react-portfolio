          ...
          <Box sx={{ mt: 5 }}>
            {data.map((job) => (
              <motion.div key={job.company} variants={itemVariants}>
                <RotatingBorderCard sx={{ mb: 3 }}> {/* Wrap with RotatingBorderCard and move mb here */}
                  {/* The Paper component is removed, directly use Box or let Card handle styling */}
                  <Box sx={{ p: 3, borderRadius: 2, background: 'none' }}> {/* Set background to none */}
